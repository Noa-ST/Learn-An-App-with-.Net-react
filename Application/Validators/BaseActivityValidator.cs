...
namespace Application.Validators
{
    public class BaseActivityValidator<T, TDto> : AbstractValidator<T> where TDto : BaseActivityDto
    {
        public BaseActivityValidator(Func<T, TDto> selector)
        {
            ...
            RuleFor(x => selector(x).Latitude).NotEmpty().WithMessage("Latidue is required.").InclusiveBetween(-90, 90).WithMessage("Latidue must be between -90 and 90.");
            RuleFor(x => selector(x).Longitude).NotEmpty().WithMessage("Longtidue is required.").InclusiveBetween(-180, 180).WithMessage("Longtidue must be between -180 and 180.");
        }
    }
}
